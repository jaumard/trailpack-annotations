...
module.exports = function (app, annotations) {
  // create the registry
  ...
  _.each(annotations, (value, key) => {
    if (app.config.annotations[key] === undefined || app.config.annotations[key]) {
      registry.registerAnnotation(path.join(__dirname, value.prototype.path))
    }
  })

  const controllersFolder = app.config.annotations.pathToScan
  const pattern = controllersFolder + path.sep + '**' + path.sep + '*Controller.js'

  const handleAnnotation = function (filePath) {
  ...
  const iterate = function (filePath) {
    // create the annotation reader
    const reader = new ecmasAnnotations.Reader(registry)

    // parse the annotations from a file
    reader.parse(filePath, ecmasAnnotations.Reader.ES6)

    // get the annotations
    const definitionAnnotations = reader.getDefinitionAnnotations()
    const constructorAnnotations = reader.getConstructorAnnotations()
    const methodAnnotations = reader.getMethodAnnotations()
    const propertyAnnotations = reader.getPropertyAnnotations()
